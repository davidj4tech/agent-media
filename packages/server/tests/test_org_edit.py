...
moving it between the GTD files (org_edit.py). Each test writes its own
...
from agent_media_server import auth, org_edit
...
def test_done_closes_on_the_planning_line_and_reopen_takes_it_off(org):
    ok, got = org_edit.set_state("inbox.org", _line("Fix the TV"), "Fix the TV ssh", "done", "good", now=NOW)
    ...
    ok, _ = org_edit.set_state("inbox.org", 4, "Fix the TV ssh", "TODO", "good", now=NOW)
    assert (org / "inbox.org").read_text() == INBOX


def test_done_without_a_planning_line_adds_one_and_keeps_priority_and_tags(org):
    ok, _ = org_edit.set_state("inbox.org", _line("Call the bank"), "Call the bank", "DONE", "good", now=NOW)
    text = (org / "inbox.org").read_text()
    assert "** DONE [#A] Call the bank :phone:\n   CLOSED: [2026-09-22 Tue 10:30]\n" in text
    org_edit.set_state("inbox.org", _line("Call the bank"), "Call the bank", "NEXT", "good", now=NOW)
    assert (org / "inbox.org").read_text() == INBOX


def test_a_repeater_moves_on_instead_of_closing(org):
    ok, got = org_edit.set_state("inbox.org", _line("Water plants"), "Water plants", "DONE", "good", now=NOW)
...
def test_repeater_kinds(stamp, after):
    line, _ = org_edit._advance(f"   SCHEDULED: {stamp}", NOW.date())
    assert line == f"   SCHEDULED: {after}"


def test_a_moved_line_is_found_by_title_and_a_gone_one_is_409(org):
    ok, got = org_edit.set_state("inbox.org", 2, "Parcel", "DONE", "good", now=NOW)
    assert ok and got["at"] == _line("Parcel")
    ok, got = org_edit.set_state("inbox.org", 4, "No such thing", "DONE", "good", now=NOW)
    assert not ok and got["status"] == 409


def test_refile_to_next_actions_moves_the_subtree_under_inbox(org):
    ok, got = org_edit.refile("inbox.org", _line("Fix the TV"), "Fix the TV ssh", "next", "good")
...
def test_refile_to_the_tickler_needs_a_date_and_schedules_it(org):
    ok, got = org_edit.refile("inbox.org", _line("Parcel"), "Parcel", "tickler", "good")
    ...
    ok, _ = org_edit.refile("inbox.org", _line("Fix the TV"), "Fix the TV ssh", "tickler",
...
def test_refile_to_someday_goes_top_level(org):
    ok, _ = org_edit.refile("inbox.org", _line("Fix the TV"), "Fix the TV ssh", "someday", "good")
    assert (org / "someday.org").read_text().startswith(
        "* TODO Fix the TV ssh\n   SCHEDULED: <2026-09-20 Sun 08:45>\n   Body about the telly.\n** a sub-point\n")


def test_a_new_date_keeps_the_time_and_the_repeater(org):
    ok, got = org_edit.set_date("inbox.org", _line("Fix the TV"), "Fix the TV ssh",
    ...
    org_edit.set_date("inbox.org", 4, "Fix the TV ssh", "scheduled", "2026-09-26", "good", time="10:00")
    assert "SCHEDULED: <2026-09-26 Sat 10:00>\n" in (org / "inbox.org").read_text()
    org_edit.set_date("inbox.org", 4, "Fix the TV ssh", "scheduled", "2026-09-26", "good", time="")
    assert "SCHEDULED: <2026-09-26 Sat>\n" in (org / "inbox.org").read_text()
    org_edit.set_date("inbox.org", _line("Water plants"), "Water plants", "scheduled", "2026-10-01", "good")
    assert "** TODO Water plants\n   SCHEDULED: <2026-10-01 Thu +1d>\n" in (org / "inbox.org").read_text()


def test_a_date_is_added_beside_the_other_or_on_a_new_line_and_taken_off(org):
    org_edit.set_date("inbox.org", 4, "Fix the TV ssh", "deadline", "2026-09-30", "good")
    text = (org / "inbox.org").read_text()
    assert "   SCHEDULED: <2026-09-20 Sun 08:45> DEADLINE: <2026-09-30 Wed>\n" in text
    ok, got = org_edit.set_date("inbox.org", 0, "Call the bank", "scheduled", "2026-09-23", "good")
    assert ok and "** NEXT [#A] Call the bank :phone:\n   SCHEDULED: <2026-09-23 Wed>\n" in (org / "inbox.org").read_text()
    org_edit.set_date("inbox.org", got["at"], "Call the bank", "scheduled", "", "good")
    org_edit.set_date("inbox.org", 4, "Fix the TV ssh", "deadline", "", "good")
    assert (org / "inbox.org").read_text() == INBOX


def test_bad_dates_are_refused(org):
    assert org_edit.set_date("inbox.org", 4, "", "scheduled", "26/9", "good")[1]["status"] == 400
    assert org_edit.set_date("inbox.org", 4, "", "closed", "2026-09-26", "good")[1]["status"] == 400
    assert org_edit.set_date("inbox.org", 4, "", "scheduled", "2026-09-26", "good", time="7pm")[1]["status"] == 400
    assert org_edit.set_date("inbox.org", 4, "", "scheduled", "2026-09-26", "bad")[1]["status"] == 401
    assert org_edit.set_date("roam/n.org", 1, "", "scheduled", "2026-09-26", "good")[1]["status"] == 400
    assert (org / "inbox.org").read_text() == INBOX


def test_edits_are_gated_and_kept_to_the_gtd_files(org):
    assert org_edit.set_state("inbox.org", 4, "", "DONE", "bad")[1]["status"] == 401
    for rel in ("roam/n.org", "../x.org", "astro.org"):
        ok, got = org_edit.set_state(rel, 1, "", "DONE", "good")
        assert not ok and got["status"] == 400
    assert org_edit.set_state("inbox.org", 4, "", "MAYBE", "good")[1]["status"] == 400
    assert org_edit.refile("inbox.org", 4, "", "inbox", "good")[1]["status"] == 400
    assert org_edit.refile("inbox.org", 4, "", "mars", "good")[1]["status"] == 400
    assert (org / "inbox.org").read_text() == INBOX
...
def test_the_routes(org):
    ...
    try:
        ...
        assert post("/org/refile", {"path": "inbox.org", "at": 1, "title": "Gone",
                                      "to": "next"})[0] == 409
    finally:
        srv.shutdown()
        srv.server_close()
...
def test_priority_is_set_changed_and_taken_off(org):
    ...
    ok, got = org_edit.set_priority("inbox.org", at, "Call the bank", "b", "good")
    assert ok and got["priority"] == "B"
    assert "** NEXT [#B] Call the bank :phone:" in (org / "inbox.org").read_text()
    ok, _ = org_edit.set_priority("inbox.org", at, "Call the bank", "", "good")
    assert "** NEXT Call the bank :phone:" in (org / "inbox.org").read_text()
    ok, _ = org_edit.set_priority("inbox.org", _line("Fix the TV ssh"), "Fix the TV ssh", "A", "good")
    assert "** TODO [#A] Fix the TV ssh" in (org / "inbox.org").read_text()
    ok, got = org_edit.set_priority("inbox.org", at, "Call the bank", "D", "good")
    assert not ok and got["status"] == 400
