"""A chat about a note (notes_chat.py): the first message names the item,
...
from agent_media_server import auth, notes_chat, send
...
def test_a_heading_is_named_and_the_chat_opens_in_the_tree(org):
    root, asked = org
    ok, got = notes_chat.ask("inbox.org", 4, "  what's blocking this?\n", "good")
...
def test_the_item_keeps_its_chats_newest_first(org):
    notes_chat.ask("inbox.org", 4, "first", "good")
    notes_chat.ask("inbox.org", 4, "second", "good")
    notes_chat.ask("inbox.org", 6, "other", "good")
    rows = notes_chat.chats("inbox.org", "Fix the TV ssh")
    assert [r["session"] for r in rows] == ["s2", "s1"] and rows[0]["title"] == "second"
    assert [r["session"] for r in notes_chat.chats("inbox.org", "Plain heading")] == ["s3"]


def test_a_whole_note_has_no_line(org):
    _, asked = org
    notes_chat.ask("inbox.org", 0, "summarise", "good")
    assert asked[0][0] == 'About "Inbox" in my Org notes (~/org/inbox.org): summarise'


def test_refusals_start_nothing(org):
    _, asked = org
    assert notes_chat.ask("inbox.org", 4, "   ", "good")[0] is False
    assert notes_chat.ask("inbox.org", 4, "hi", "bad")[1]["status"] == 401
    assert notes_chat.ask("../etc/passwd", 0, "hi", "good")[1]["status"] == 404
    assert notes_chat.ask("inbox.org", 5, "hi", "good")[1]["status"] == 409
    assert asked == []
