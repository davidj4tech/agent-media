...
`am start` intent, ssh'd into the phone (p8a). The default video handler
...
- Commands must `ssh p8a` first — bare relay commands run on red5, not the phone.
...
- Working launch shape (VLC component):
  ssh p8a 'am start -n org.videolan.vlc/.gui.video.VideoPlayerActivity \
