"""/speech/now: the app's speech bar, named and gated.

The /speech/ctl whitelist is checked against the canvas's own verbs in
packages/visual/tests/test_speech_ctl.py."""

from __future__ import annotations

from agent_media_server import auth_abs, sessions, share, speech, threads
...
def test_live_speech_is_named(monkeypatch):
    _as(monkeypatch)
    monkeypatch.setattr(sessions, "sessions_index",
                        lambda: [{"session": SID, "title": "Sasonica music", "live": True}])
    monkeypatch.setattr(threads, "item_for_session", lambda s, b: ("li_42", True))
    ok, out = speech.speech_now("tok", {"speaking": True, "session": SID,
...
def test_paused_speech_is_still_live(monkeypatch):
    ...
    ok, out = speech.speech_now("tok", {"speaking": False, "paused": True, "session": SID})
...
def test_quiet_names_nothing(monkeypatch):
    _as(monkeypatch)
    called = []
    monkeypatch.setattr(threads, "item_for_session", lambda s, b: called.append(s))
    ok, out = speech.speech_now("tok", {"speaking": False, "session": SID})
...
def test_an_unready_item_is_not_offered(monkeypatch):
    _as(monkeypatch)
    monkeypatch.setattr(sessions, "sessions_index", lambda: [])
    monkeypatch.setattr(threads, "item_for_session", lambda s, b: ("li_42", False))
    _, out = speech.speech_now("tok", {"speaking": True, "session": SID})
...
def test_the_title_is_asked_once_per_reply(monkeypatch):
    _as(monkeypatch)
    asks = []
    monkeypatch.setattr(sessions, "sessions_index", lambda: asks.append(1) or [])
    monkeypatch.setattr(threads, "item_for_session", lambda s, b: (None, False))
    for _ in range(3):
        speech.speech_now("tok", {"speaking": True, "session": SID})
    assert len(asks) == 1


def test_a_stranger_is_refused(monkeypatch):
    _as(monkeypatch, user=None, status=401)
    ok, out = speech.speech_now("", {"speaking": True, "session": SID})
    assert not ok and out.get("status") in (401, 403)
    assert not auth_abs.may_control_speech("")[0]


def test_speed_and_mute_ride_along(monkeypatch):
    _as(monkeypatch)
    ok, out = speech.speech_now("tok", {"speaking": True, "speed": 1.6, "muted": True})
...
def test_session_states_are_swept_once_per_ttl(monkeypatch):
    _as(monkeypatch)
    sweeps = []
    monkeypatch.setattr(sessions, "_live_states", lambda: sweeps.append(1) or [])
    monkeypatch.setattr(sessions, "_STATES_CACHE", (0.0, []))
    for _ in range(3):
        sessions.session_states("tok")
    assert len(sweeps) == 1


def test_session_states_refuse_a_stranger(monkeypatch):
    _as(monkeypatch, user=None, status=401)
    ok, _ = sessions.session_states("")
    assert not ok
...
def test_a_share_from_the_app_is_gated_and_dispatched(monkeypatch):
    ...
    _as(monkeypatch, user=None, status=401)
    ok, out = share.share_from_app("https://example.com/x", "", "")
    assert not ok and out.get("status") in (401, 403)
    ...
    ok, out = share.share_from_app("look https://example.com/x", "banana", "tok")
...
def test_nothing_shared_is_said_so(monkeypatch):
    _as(monkeypatch)
    ok, out = share.share_from_app("   ", "", "tok")
