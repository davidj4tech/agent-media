...
Status: **decided 24 Sep 2026** (open questions answered as recommended); **steps 1 and 2 built the same day** (the profile seam and `packages/notes-paragtd`; agenda files, `#+TODO:` keywords, the Emacs import), step 3 (the `/notes` fields, and Next reading them), and step 4's sequencing (Org's blocking in core, paragtd's trigger in its package). The manifest is still to come. Changes §6.10 of `server-contract.md` (new
